      ...
      <div className="container px-5 md:py-24 pt-40 pb-28 mx-auto">
        <div className="flex flex-wrap w-full mb-20">
          <div className="lg:w-1/2 w-full mb-6 lg:mb-0">
            <h1 className="sm:text-3xl text-2xl font-medium title-font mb-2 text-white">{genre}</h1>
          ...
          {movies.length > 0 ? (
            movies.map((movie, idx) => (
              <div key={idx} className="xl:w-1/4 md:w-1/2 p-4">
                <div className="bg-gray-800 bg-opacity-40 p-6 rounded-lg">
                  ...
                  <h3 className="tracking-widest text-indigo-400 text-xs font-medium title-font">{movie.type}</h3>
                  <h2 className="text-lg text-white font-medium title-font mb-4">{movie.title}</h2>
                  <p className="leading-relaxed text-base">{limitWords(movie.shortDesc, 20)}</p>
                </div>
              </div>
            ))
          ) : (
            <div className="text-gray-400 body-font bg-gray-900 h-screen m-auto">
              <div className="bg-gray-800 text-white p-8 rounded-lg shadow-lg mx-auto">
                <p className="text-2xl font-bold mb-4">No movies found for this genre.</p>
                <p className="text-lg">Please check the genre or try a different one.</p>
              </div>
